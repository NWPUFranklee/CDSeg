...
class CATSeg(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.attn_refine = diffusion(
                ...
                time_step=45,
                device=torch.device,
                dtype=torch.float16 if "cuda" in str(torch.device) else torch.float32
            )
    ...
    def forward(self, batched_inputs):
        ...
        outputs = self.attn_refine(clip_images_resized, outputs)
    ...
    # 添加 refinement 函数及其依赖
    import math

    # 辅助函数: weighted_attention_fusion_by_entropy_joint
    def weighted_attention_fusion_by_entropy_joint(
        A_self_list: List[torch.Tensor],
        A_cross: torch.Tensor,
        alpha: float = 3.0,
        eps: float = 1e-12
        ...
        entropies = []
        A_cross = A_cross.to(A_self_list[0].dtype)
        for A_self in A_self_list:
            P = A_self @ A_cross
            P = P / (P.sum(dim=-1, keepdim=True) + eps)
            H_row = -(P * (P + eps).log()).sum(dim=-1)
            entropies.append(H_row.mean())
        entropies = torch.stack(entropies)

        raw_w  = torch.exp(-alpha * entropies / logK)
        weights = raw_w / raw_w.sum()
        A_weighted = sum(w * A_self for w, A_self in zip(weights, A_self_list))
        return A_weighted, weights

    # 辅助函数: chained_attention_product
    def chained_attention_product(
        qk_store: List[Dict[str, torch.Tensor]],
        layers: List[int],
        A_cross: torch.Tensor,
        d: int = 64
    ) -> torch.Tensor:
        ...
        for idx in layers:
            q = qk_store[idx]["query"]
            k = qk_store[idx]["key"]
            for h in range(q.shape[0]):
                qh, kh = q[h].float(), k[h].float()
                H = W = int(math.sqrt(qh.shape[0]))

                M = build_cosine_local_mask(qh, kh, H, W, kernel)
                M += torch.eye(M.size(0), device=M.device) * 1e-2

                logits = (qh @ kh.T) / math.sqrt(d)
                A_l = torch.softmax(logits * M , dim=-1)
                A_list_l.append(A_l)

                A_g = torch.softmax(logits, dim=-1)
                A_list_g.append(A_g)

        def preprocess(A_list):
            A = torch.stack(A_list, dim=0)
            A = A / (A.amax(dim=-2, keepdim=True) + 1e-5)
            A = A + torch.where(A == 0, 0, 0.01 * (torch.log10(torch.e * A)))
            A = torch.clamp(A, min=0)
            A = A / (A.sum(dim=-1, keepdim=True) + 1e-5)
            return list(A.unbind(dim=0))

        A_proc_g = preprocess(A_list_g)
        A_proc_l = preprocess(A_list_l)

        A_w_g, _ = weighted_attention_fusion_by_entropy_joint(A_proc_g, A_cross)
        A_w_l, _ = weighted_attention_fusion_by_entropy_joint(A_proc_l, A_cross)
        return 0.8 * A_w_g + (1 - 0.8) * A_w_l

    # 添加到 CATSeg 类
    def refinement(self, ori_img, pred_mask):
        # if self.attn_refine is None:
        #     return pred_mask

        pred = F.interpolate(pred_mask[None], size=(64,64),
                             mode='bilinear', align_corners=False)[0]
        cross_att = pred.flatten(-2).T

        self.attn_refine(ori_img.to(self.device), "")
        self_att = torch.cat([self.attn_refine.attention_maps[idx][0]
                              for idx in self.cfg.attention_layers_to_use],
                             dim=0).float()
        self_att /= self_att.amax(dim=-2, keepdim=True) + 1e-5
        self_att = torch.where(self_att < 0.1, 0, self_att)
        self_att /= self_att.sum(dim=-1, keepdim=True) + 1e-5

        if self.cfg.refinement == "mean":
            fused = self_att.mean(0)
        elif self.cfg.refinement == "selection":
            fused = self_att[self.cfg.attention_idx]
        else:
            fused = chained_attention_product(
                self.attn_refine.qk_store,
                self.cfg.attention_layers_to_use,
                cross_att,
                d=64
            )

        fused = fused.to(fused.dtype)
        cross_att = cross_att.to(fused.dtype)
        refined = cross_att.clone()
        A_power = torch.eye(fused.size(0),
                            device=self.device,
                            dtype=fused.dtype)

        N = fused.size(0)
        I = torch.eye(N, device=fused.device, dtype=fused.dtype)

        # 1) compute (I - Δ·F)^{-1}
        inv_term = torch.linalg.inv(I - 0.9 * fused)

        # 2) form the normalized infinite random‐walk
        refined_inf = (1 - 0.9) * (inv_term @ cross_att)

        # 3) if you still want the final fused step:
        out = fused @ refined_inf

        # 4) reshape back to map form
        return out.T.reshape(-1, 64, 64)

# 修复未定义的依赖问题
import math

# 添加 build_cosine_local_mask 函数
def build_cosine_local_mask(
    q: torch.Tensor,
    k: torch.Tensor,
    H: int, W: int,
    kernel: torch.Tensor,
    ...
    N, D = q.shape
    assert N == H * W
    kernel = kernel.to(q.device)

    coords = torch.stack(torch.meshgrid(
        torch.arange(H, device=q.device),
        torch.arange(W, device=q.device),
        indexing="ij"
    ), dim=-1).view(-1, 2)

    offsets = [
        (i - kernel.shape[0]//2, j - kernel.shape[1]//2)
        for i in range(kernel.shape[0])
        for j in range(kernel.shape[1])
        if kernel[i, j] != 0
    ]

    M = torch.zeros((N, N), device=q.device, dtype=q.dtype)
    for dy, dx in offsets:
        nbr = coords + torch.tensor([dy, dx], device=q.device)
        valid = (
            (nbr[:, 0] >= 0) & (nbr[:, 0] < H) &
            (nbr[:, 1] >= 0) & (nbr[:, 1] < W)
        )
        idx0 = (coords[valid][:, 0] * W + coords[valid][:, 1]).long()
        idx1 = (nbr[valid][:, 0] * W   + nbr[valid][:, 1]).long()
        raw_sim = F.cosine_similarity(q[idx0], k[idx1], dim=-1)
        sim = (raw_sim + 1.0) * 0.5 + eps
        M[idx0, idx1] = sim
    return M
